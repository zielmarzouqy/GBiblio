...
public class MainCollection {
	...
	public static boolean emprunter(String idDocument, String idAdherent) {
		...
		Adherent adherent = listAdherent.get(idAdherent);
		Document doc = listDocument.get(idDocument);

		if (adherent != null && doc != null && ((Livre) doc).isDisponible()) {			
			Set<Emprunt> emprunt = new HashSet<Emprunt>();
			emprunt.addAll(listAdherent.get(idAdherent).getSetEmprunt());
			...
			emprunt.add(new Emprunt(new Date(), date_2, doc));
			listAdherent.get(idAdherent).setSetEmprunt(emprunt);
			((Livre) doc).setDisponible(false);
